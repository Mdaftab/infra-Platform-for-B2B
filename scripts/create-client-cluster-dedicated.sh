#!/bin/bash
# Script to create a new client cluster with dedicated VPC (no shared VPC)
...
# Banner
echo -e "${BOLD}╔════════════════════════════════════════════════════════════╗${NC}"
echo -e "${BOLD}║  Client GKE Cluster Creation Tool (Dedicated VPC)          ║${NC}"
echo -e "${BOLD}╚════════════════════════════════════════════════════════════╝${NC}"
echo -e "This script helps you create a new client-specific GKE cluster with a dedicated VPC."
echo
...
create_client_resources() {
  ...
  # Create client cluster claims with dedicated VPC
  TEMPLATE_FILE="../crossplane/xresources/client-gke-cluster-dedicated-template.yaml"
...
# Function to set up cloud SQL database if needed
setup_cloud_sql() {
  echo -e "${YELLOW}Would you like to set up a Cloud SQL database for this client? (y/n)${NC}"
  read -p "> " SETUP_DB
  
  if [[ "$SETUP_DB" == "y" || "$SETUP_DB" == "Y" ]]; then
    echo -e "${YELLOW}Setting up Cloud SQL database...${NC}"
    
    # Create Cloud SQL instance configuration
    cat <<EOF > ../crossplane/xresources/$CLIENT_NAME-cloudsql-claim.yaml
...
spec:
  forProvider:
    ...
    settings:
      ...
      ipConfiguration:
        ipv4Enabled: false
        privateNetworkRef:
          name: ${CLIENT_NAME}-gke-cluster-network
      backupConfiguration:
        enabled: true
        startTime: "03:00"
  ...
  writeConnectionSecretToRef:
    ...
    name: ${CLIENT_NAME}-db-connection
EOF

    # Apply the database claim
    kubectl --context=infracluster apply -f ../crossplane/xresources/$CLIENT_NAME-cloudsql-claim.yaml
    
    echo -e "${GREEN}Cloud SQL database setup initiated for $CLIENT_NAME.${NC}"
  else
    echo -e "${YELLOW}Skipping Cloud SQL database setup.${NC}"
  fi
}
...
main() {
  ...
  if [[ "$SETUP_DB" == "y" || "$SETUP_DB" == "Y" ]]; then
    echo -e "${GREEN}✅ Cloud SQL database being provisioned for: $CLIENT_NAME${NC}"
  fi
  echo
